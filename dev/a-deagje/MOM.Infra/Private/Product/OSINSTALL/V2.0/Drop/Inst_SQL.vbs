...
'          (12/09/2003) Dean Gjedde - Change usage example - trival change
'          (02/28/2004) Dean Gjedde - Now create inst_sql.log in %systemdrive%\logs
'          (03/01/2004) Dean Gjedde - Added support for detecting install OS language
...
' -------------------------------------------------------------------
' summary: create the SQL Server 2000 reg key install hack for W2K3
' -------------------------------------------------------------------
...
' -------------------------------------------------------------------
' summary: Get the build number for the windows install
' -------------------------------------------------------------------
...
' -------------------------------------------------------------------
Function GetOSLang()
    Dim oLocator, oServices, oInfoSet, oInfo, iOSNum, sOSLang
    ...
    For Each oInfo In oInfoSet
        iOSNum = oInfo.OSLanguage
    Next
    ' http://www.microsoft.com/globaldev/reference/winxp/xp-lcid.mspx
    Select Case CInt(iOSNum)
...
Sub WriteLog(sMessage, sLog, iExitCode)
    ...
    Dim sLogFile, oFSO, oStream, oShell, sWINDIR
...
' -------------------------------------------------------------------
' summary: the ReplaceFileTxt function will replace text in a file and write
'    it to the new file
' -------------------------------------------------------------------
...
' -------------------------------------------------------------------
' summary: sets a service's startup type
' -------------------------------------------------------------------
...
' -------------------------------------------------------------------
' summary: stops or starts a service
' -------------------------------------------------------------------
...
Sub Usage()
    ...
    WScript.Echo(vbTab & "If not specificed Inst_SQL.vbs will detect to OS Language")
...
Sub Main()
    ...
    Const sSQLSADefaultPassword = "SMX#2001"

    Dim oShell, oEnv, aSplit, aArguments, aDASUser, aCAMUser, sArg, sLog
    ...
    sLog = sSystemDrive & "\logs\Inst_SQL.Log"
    ...
    If sSAPassword = "" Then
        ...
        If sSAPassword = ""  Then
            sSAPassword = sSQLSADefaultPassword
        End If
    End If
    ...
    Else
        ...
        Else
            WriteLog "VAR_ABORT - " & sSQLRTMIssFile & " does not exist or is not accessible", sLog, iPathNotFoundError
        End If
        If oFSO.FileExists(sSQLSP3IssFile) Then
            oFSO.CopyFile sSQLSP3IssFile, sTempDir & "\", bOverRide
        Else
            WriteLog "VAR_ABORT - " & sSQLSP3IssFile & " does not exist or is not accessible", sLog, iPathNotFoundError
        End If
        ...
        If(CheckForSQL) Then
            ...
            Else
                WriteLog "VAR_ABORT - " & sSQLRTMCustIssFile & " does not exist or is not accessible", sLog, iPathNotFoundError
            End If
    ...
    If Not(oFSO.FileExists(sCommand)) Then
        WriteLog "VAR_ABORT - " & sCommand & " does not exist or is not accessible", sLog, iPathNotFoundError
    End If
    If Not sSPCommand = "" Then
        If Not(oFSO.FileExists(sSPCommand)) Then
            WriteLog "VAR_ABORT - " & sSPCommand & " does not exist or is not accessible", sLog, iPathNotFoundError
        End If
    End If
