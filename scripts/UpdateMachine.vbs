...
Sub Main()
    ...
    strOwner = WScript.Arguments(1)
      
	' -----------------------------------------------------------------------------------
	' comment:
	'    Check if the machine is in machinelib
	' -----------------------------------------------------------------------------------
	Set objXMLData = GetMachineLibInfo(strCompName)
	If objXMLData.xml = "<root></root>" & vbCrlf Then
	    WScript.Echo("Computer: " & strCompName & " has no entry in machinelib")
	Elseif Instr(Ucase(objXMLData.xml), "ERROR") Then
	    ...
	    End If
	Else
	    WScript.Echo("Computer: " & strCompName & " already has an entry in machinelib")
	End If

	WScript.Echo(objXMLData.xml)
	
	Set objXMLData = Nothing
	Set objMachNode = Nothing
	
	strMessage = "<root><MACHINE action=""autoupdate"" name=""" + strCompName + _
	""" owner=""" + strOwner + """></MACHINE></root>"
	WScript.Echo("strMessage is " & strMessage)
	Set objXMLData = UpdateMachine(strMessage)
    If Instr(Ucase(objXMLData.xml), "ERROR") Then
	    ...
	    End If
	Else
	    WScript.Echo("Computer: " & strCompName & " entry was updated")
	End If
